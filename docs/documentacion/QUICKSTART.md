...
- **Toda aprobación de documentos y lotes deja sello** (RDD): al aprobar, el gate corre `sello.py` — estampa `> Aprobado por [nombre] — fecha` en el documento y genera un receipt con el hash sha256 del contenido aprobado; si el documento cambia después, la fase siguiente se bloquea (marcar checkboxes `[x]` NO cuenta como cambio; re-sellar exige `--re-sellar`, nunca es silencioso). En build, el lote validado se sella con un tag de git sobre el commit exacto y **el tag viaja en el mismo push que el código**; el hook pre-push revisa commit por commit y bloquea el código sin sellar — los pushes de solo documentación pasan, y el candado se activa con el primer sello. El mismo hook trae un **candado de secretos** que corre siempre, con o sin sellos y docs incluidos: una credencial con pinta real bloquea el push; un falso positivo se exime marcando la línea con `stark:no-secreto`. La aprobación del **prototipo** es distinta por diseño: la da el cliente y queda en el validation-log. Sin sello no hay aprobación: es el recibo que tu repo demuestra sin el chat. (RDD es **experimental**: candado contra el descuido y la deriva — `--no-verify` lo salta y el hook es por clon; límites completos en el README.)
