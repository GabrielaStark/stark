...
5. Marcas `[x]` solo las que pasen (marcarlas NO invalida el sello del `tasks.md`: el estado de los checkboxes no es contenido aprobado), y el gate sella el commit validado: `python3 .claude/scripts/sello.py sellar-lote <id> --por "<nombre>"` — `<n>` en construcción, `<feature>-<n>` en mantenimiento, con `--invariantes` si esa validación pasó. Es un **annotated tag** `stark-lote-<id>` sobre el commit exacto, con working tree limpio como requisito. El hook pre-push (lo instala `/stark-init` o el primer `/stark-build`) revisa **cada commit de cada ref empujada** y bloquea código sin sellar; el tag debe viajar en el mismo push o ya existir en el remoto: `git push <remoto> <rama> refs/tags/stark-lote-<id>`. Si algo cambió tras la validación: se revalida, no se entrega. Cierras la conversación. Abres otra para el siguiente lote.
...
- **RDD** (Receipt-Driven Development): ninguna validación sin evidencia verificable — el agente no dice "ya quedó", lo demuestra el repo. stark lo implementa con los sellos de aprobación y su autoridad ejecutable: `.claude/scripts/sello.py`.
- **Sello de aprobación:** la evidencia que deja cada gate, con autoridad ejecutable (`sello.py`), no solo una línea de Markdown. De documento: `> Aprobado por [nombre] — fecha` en el header **+ receipt JSON con sha256 del contenido aprobado** (atado a la ruta canónica, en `docs/.stark/receipts/`) — la fase siguiente recalcula el hash (`verificar-doc`) y se bloquea si el documento cambió tras aprobarse; marcar checkboxes `[x]` no cuenta como cambio. De lote: **annotated tag** `stark-lote-<id>` sobre el commit validado (exige working tree limpio; inmutable vía push — la protección server-side sigue pendiente). El hook pre-push (`verificar-push`) revisa **cada commit de cada ref empujada** contra su remoto: bloquea código sin sellar (por otra rama, tras un revert o renombrado a docs/ también) y exige que el tag viaje en el mismo push o exista ya en el remoto; los pushes de solo documentación pasan, y el candado se activa con el primer sello (un clon con receipts lo hereda al copiar la herramienta e instalar el hook). El mismo hook monta el **candado de secretos**: escanea cada commit empujado con patrones de alta precisión (llaves cloud, tokens, private keys, credencial literal asignada) — corre siempre, con o sin sellos y docs incluidos; un falso positivo se exime con `stark:no-secreto` en la línea. El primer sello del repo deja además la **firma de origen** `docs/.stark/STARK.md` (una sola vez; si se elimina, no se regenera). Sin receipt/tag válido, no hay aprobación. RDD es **experimental**: candado contra el descuido y la deriva, no contra la mala fe (`--no-verify` lo salta; el hook es por clon) — límites completos en el README. Los escenarios completos son reproducibles: `python3 scripts/test_sello.py`.
