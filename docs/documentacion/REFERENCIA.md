...
- **Sello de aprobación:** la evidencia que deja cada gate, con autoridad ejecutable (`sello.py`), no solo una línea de Markdown. De documento: `> Aprobado por [nombre] — fecha` en el header **+ receipt JSON con sha256 del contenido aprobado** (atado a la ruta canónica, en `docs/.stark/receipts/`) — la fase siguiente recalcula el hash (`verificar-doc`) y se bloquea si el documento cambió tras aprobarse; marcar checkboxes `[x]` no cuenta como cambio. De lote: **annotated tag** `stark-lote-<id>` sobre el commit validado (exige working tree limpio; inmutable — ni borrarlo ni moverlo). El hook pre-push (`verificar-push`) revisa **cada commit de cada ref empujada** contra su remoto: bloquea código sin sellar (por otra rama, tras un revert o renombrado a docs/ también) y exige que el tag viaje en el mismo push o exista ya en el remoto; los pushes de solo documentación pasan, y el candado se activa con el primer sello (un clon con receipts lo hereda activo). Sin receipt/tag válido, no hay aprobación. Los escenarios completos son reproducibles: `python3 scripts/test_sello.py`.
