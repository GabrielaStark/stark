...
Pre-condición: `tasks.md` debe estar aprobado. Verifícalo: `python3 .claude/scripts/sello.py verificar-doc <ruta-del-tasks>`. Si falla, detente: se re-aprueba y re-sella antes de ejecutar. La primera vez en el repo, instala también el candado de push: `python3 .claude/scripts/sello.py instalar-hook`.
...
SELLO DEL LOTE (cadena de evidencia): cuando el humano aprueba el lote, deja el working tree limpio (todo commiteado, receipts incluidos) y sella el commit validado: `python3 .claude/scripts/sello.py sellar-lote <n> --por "<nombre>"` (en mantenimiento agrega `--invariantes` solo si la validación de invariantes pasó). El sello es un **annotated tag** `stark-lote-<n>` sobre el commit exacto — no modifica el árbol validado, así que no se invalida a sí mismo. El push se protege solo: el hook pre-push corre `verificar-push` (working tree limpio, sin untracked, HEAD sellado, `Tests: PASS` registrado) y bloquea si algo cambió después de la validación — se revalida, no se entrega. Pushea rama y tags juntos: `git push origin <rama> --tags`.
