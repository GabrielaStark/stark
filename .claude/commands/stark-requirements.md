...
Gate humano: nada avanza a la siguiente fase sin aprobación explícita del requirements producido. Al recibirla, sella el documento: `python3 .claude/scripts/sello.py sellar-doc <ruta-del-requirements> --por "<nombre>"` (pregunta el nombre si no lo tienes; si el documento ya tenía receipt — válvula de retorno, corrección re-aprobada — agrega `--re-sellar`). Eso estampa `> Aprobado por ...` en el header y genera el receipt (hash sha256 del contenido aprobado) en `docs/.stark/receipts/` — commitea documento y receipt juntos. Sin receipt válido, el documento no cuenta como aprobado.
