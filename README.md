...
El mismo hook monta además un **candado de secretos**: cada commit que se empuja se escanea contra patrones de alta precisión (llaves cloud, tokens, private keys, credencial literal asignada). Corre siempre — con o sin sellos — y también sobre `docs/`, donde la exención RDD no aplica: una credencial pegada en un requirements es una fuga igual. Un falso positivo se exime marcando la línea con `stark:no-secreto`; un secreto real no se borra, se **rota** — git no olvida. Límites, de frente: el diff propio de un merge no se escanea y los patrones priorizan precisión sobre exhaustividad — para una auditoría profunda de historial usa una herramienta dedicada (p. ej. gitleaks).
...
**La firma de origen**: junto con esa evidencia viaja una sola pieza de atribución — el primer sello del repo genera `docs/.stark/STARK.md`, una nota única que dice que el proyecto se elaboró con stark (herramienta de **@iamgabstark_**, con link a este repo) y le explica a quien lo mantenga qué son los receipts y los tags y cómo verificarlos. Deliberadamente no invasiva: un solo archivo junto a los receipts, ni una línea en tu código ni en tu README, y una sola vez — si la eliminas, no se regenera.
